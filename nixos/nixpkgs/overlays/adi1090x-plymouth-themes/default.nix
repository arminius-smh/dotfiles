...
let
  cfg = config.cave.nixpkgs.overlays.adi1090x-plymouth-themes;
in
{
  options.cave = {
    nixpkgs.overlays.adi1090x-plymouth-themes.enable = lib.mkEnableOption "enable nixpkgs.overlays.adi1090x-plymouth-themes config";
  };
